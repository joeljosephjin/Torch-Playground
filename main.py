...
def get_args():
    ...
    parser.add_argument(
        "--model", type=str, default="SimpleModel", help="SimpleModel or AVModel,.."
    )
    parser.add_argument(
        "--dataset", type=str, default="cifar_10", help="cifar_10 or mnist,.."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="name of the exp to load from",
    )
    parser.add_argument(
        "--save-as", type=str, default="", help="a name for the model save file"
    )
...
class ClassifierPipeline:
    ...
    def __init__(self, args=None, net=None, datatuple=None):
        ...
        if self.args.resume_from_saved:
            self.net = self.load_model(
                filename=self.args.model + self.args.resume_from_saved,
    ...
    def train(self, epochs=3):
        ...
        for epoch in range(self.args.epochs):  # loop over the dataset multiple times; 4
            ...
            for i, data in enumerate(self.trainloader):
                ...
                if (
                    i % logs_interval == 0
                ):  # and i != 0:    # print every 2000 mini-batches
                    print(
                        f"[{epoch + 1}, {i + 1:5d}] loss: {sum(running_loss) / len(running_loss):.3f} acc: {sum(running_acc) / len(running_loss):.3f} acc_densenet: {sum(running_accs_densenet) / len(running_accs_densenet):.3f} time: {(time() - self.start_time) / 60:.2f} minutes"
                    )
                    running_loss = []
                    running_acc = []
                ...
                wandb.log(
                    {
                        ...
                        "train_acc": acci_densenet.item(),
                        "model_param": list(self.net.parameters())[0][0][0][0][
                            0
                        ].item(),
                    }
                )
    ...
    def test(self, epoch):
        ...
        print(
            f"Accuracy of the network on the 10000 test images: {test_accuracy} %, acc_densenet is {acc_densenet} %"
        )
        if self.args.use_wandb:
